    ...
    <div className='h-screen w-full bg flex justify-end items-center'>
      {!show ? (
        <div className='h-[90vh] w-[35vw] bgform mr-[10%] bg-[#dce0e0] p-6 rounded-md xl:h-[75vh] xl:w-[27vw] xl:mr-[15%]'>
          ...
          <form className='flex flex-col gap-[2vw] mt-3' onSubmit={handleSubmit}>
            ...
            <div className='flex justify-center mt-5 xl:mt-10'>
              <button type="submit" className='bg-[#278986] py-2 w-[70%] rounded-full text-white font-pop xl:py-3'>Submit</button>
            </div>
          </form>
        </div>
      ) : (
        <div className='h-[90vh] w-[35vw] mr-40 bg-[#dce0e0] p-6 rounded-md flex flex-col items-center gap-8 relative xl:h-[75vh] xl:w-[27vw] xl:mr-[15%] xl:gap-[2vw]'>
          <IoMdArrowRoundBack onClick={() => setShow(false)} className='justify-self-start absolute top-2 left-2 h-8 w-8 cursor-pointer' />
          <div className='text-2xl font-play font-semibold '>Best Crops As Per Given Data</div>
          <img src={cropsimages.predictedCrop} alt="" className='h-[50%] w-[80%] xl:h-[60%] xl:w-[90%]' />
          <div className='text-4xl font-play font-semibold'>{predictedCrop}</div>
        </div>
      )}
    </div>
  );
